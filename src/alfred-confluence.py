...
def getConfluenceBaseUrl():
    if wf.settings.get(PROP_BASEURL):
        ...
        wf.add_item(
            title="No Confluence Base URL set.",
...
def _encode_creds(config):
    userpass = config["username"] + ":" + config["password"]
    return b64encode(userpass.encode()).decode()
...
def checkConfig(config):
    ...
    r = web.get(
        ...
        headers=dict(Authorization="Basic " + _encode_creds(config)),
    )
...
def main(wf):
    ...
    # query Confluence
    r = web.get(
        config["baseUrl"] + "/rest/quicknav/1/search",
...
if __name__ == "__main__":
    wf = Workflow(update_settings={"github_slug": "skleinei/alfred-confluence", "version": VERSION})
    ...
    if wf.update_available:
        ...
        wf.add_item(
            "New version of the Alfred Confluence workflow available",
