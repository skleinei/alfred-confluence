...
class Settings(dict):
    ...
    def _load(self):
        ...
        with open(self._filepath, "rb") as file_obj:
            for key, value in json.load(file_obj).items():
                d[key] = value
        self.update(d)
    ...
    def save(self):
        ...
        with LockFile(self._filepath):
            with atomic_writer(self._filepath, "w") as file_obj:
...
class Workflow:
        ...
        **Note:** all values are Unicode strings except ``version_build`` and
        ...
        for key in (
            ...
            if isinstance(value, str):
                if key in ("alfred_debug", "alfred_version_build", "alfred_theme_subtext"):
                    value = int(value)
                else:
    ...
    def _default_cachedir(self):
        ...
        return os.path.join(
            os.path.expanduser("~/Library/Caches/com.runningwithcrayons.Alfred-2/" "Workflow Data/"), self.bundleid
        )
    ...
    def _default_datadir(self):
        ...
        return os.path.join(os.path.expanduser("~/Library/Application Support/Alfred 2/Workflow Data/"), self.bundleid)
    ...
    def workflowdir(self):
        ...
        if not self._workflowdir:
            ...
            candidates = [os.path.abspath(os.getcwd()), os.path.dirname(os.path.abspath(os.path.dirname(__file__)))]
    ...
    def logger(self):
        ...
        if not len(logger.handlers):  # Only add one set of handlers
            ...
            fmt = logging.Formatter(
                "%(asctime)s %(filename)s:%(lineno)s" " %(levelname)-8s %(message)s", datefmt="%H:%M:%S"
            )
    ...
    def settings(self):
        ...
        if not self._settings:
            self.logger.debug("Reading settings from `{}` ...".format(self.settings_path))
    ...
    def cache_serializer(self, serializer_name):
        ...
        if manager.serializer(serializer_name) is None:
            raise ValueError(
                "Unknown serializer : `{}`. Register your serializer " "with `manager` first.".format(serializer_name)
            )

        self.logger.debug(f"default cache serializer set to `{serializer_name}`")
    ...
    def data_serializer(self, serializer_name):
        ...
        self.logger.debug(f"default data serializer set to `{serializer_name}`")
    ...
    def stored_data(self, name):
        ...
        if not os.path.exists(metadata_path):
            self.logger.debug(f"No data stored for `{name}`")
            return None
        ...
        self.logger.debug("Data `{}` stored in `{}` format".format(name, serializer_name))
        ...
        self.logger.debug(f"Stored data loaded from : {data_path}")
    ...
    def store_data(self, name, data, serializer=None):
        ...
        def delete_paths(paths):
            ...
            for path in paths:
                if os.path.exists(path):
                    os.unlink(path)
                    self.logger.debug(f"Deleted data file : {path}")
        ...
        self.logger.debug(f"Stored data saved at : {data_path}")
    ...
    def cached_data(self, name, data_func=None, max_age=60):
        ...
        cache_path = self.cachefile("{}.{}".format(name, self.cache_serializer))
        age = self.cached_data_age(name)

        if (age < max_age or max_age == 0) and os.path.exists(cache_path):

            with open(cache_path, "rb") as file_obj:
                self.logger.debug("Loading cached data from : %s", cache_path)
    ...
    def run(self, func, text_errors=False):
        ...
        try:
            ...
            if self.version:
                self.logger.debug(f"Workflow version : {self.version}")
            ...
            if self.help_url:
                self.logger.info(f"For assistance, see: {self.help_url}")
            ...
            self.logger.debug("Workflow finished in {:0.3f} seconds.".format(time.time() - start))
    ...
    def send_feedback(self):
        ...
        sys.stdout.write(ET.tostring(root).decode())
        sys.stdout.flush()
    ...
    def last_version_run(self):
        ...
        self.logger.debug("Last run version : {}".format(self._last_version_run))
    ...
    def set_last_version(self, version=None):
        ...
        self.logger.debug(f"Set last run version : {version}")
    ...
    def check_update(self, force=False):
        ...
        if force or not self.cached_data_fresh("__workflow_update_status", frequency * 86400):
            ...
            cmd = ["/usr/bin/python3", update_script, "check", github_slug, version]
    ...
    def start_update(self):
        ...
        cmd = ["/usr/bin/python3", update_script, "install", github_slug, version]
